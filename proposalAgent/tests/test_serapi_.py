"""
主要工具：
- scholar_article_brief
- scholar_resolve_author
- scholar_author_citations_auto
- scholar_author_citations
- scholar_author_articles_citations (可选)
"""
...
def resolve_author_candidates(input: ResolveAuthorInput) -> List[Dict[str, Any]]:
    client = SerpAPIScholar(hl=input.hl)
...
def get_author_citations(input: AuthorCitationsInput) -> Dict[str, Any]:
    ...
    if not author_id and input.name:
        ...
        cands = resolve_author_candidates(
            ResolveAuthorInput(name=input.name, organization=input.organization, hl=input.hl)
        )
        if cands:
...
def get_author_citations_auto(input: AuthorCitationsAutoInput) -> Dict[str, Any]:
    ...
    best = cands[0]
    metrics = get_author_citations(AuthorCitationsInput(author_id=best["author_id"], hl=input.hl))
...
def get_author_articles_citations(input: AuthorArticlesInput) -> List[Dict[str, Any]]:
    client = SerpAPIScholar(hl=input.hl)
...
try:
    ...
    def to_langchain_tools():
        return [
            ...
            StructuredTool.from_function(
                func=resolve_author_candidates,
                name="scholar_resolve_author",
                description="依据姓名/机构/代表作题名解析 Google Scholar author_id，返回候选列表与打分。",
                args_schema=ResolveAuthorInput,
                return_direct=False,
            ),
            StructuredTool.from_function(
                func=get_author_citations_auto,
                name="scholar_author_citations_auto",
                description="无 author_id 时的首选：先解析候选 author_id，再返回被引/h-index 指标。",
                args_schema=AuthorCitationsAutoInput,
                return_direct=False,
            ),
            StructuredTool.from_function(
                func=get_author_citations,
                name="scholar_author_citations",
                description="已有 author_id 时直接获取被引/h-index 指标。",
                args_schema=AuthorCitationsInput,
                return_direct=False,
            ),
            StructuredTool.from_function(
                func=get_author_articles_citations,
                name="scholar_author_articles_citations",
                description="（可选）列出作者每篇文章及其被引数，按被引降序返回。",
                args_schema=AuthorArticlesInput,
                return_direct=False,
            ),
        ]
except Exception:
