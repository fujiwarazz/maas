...
from proposalAgent.tests.simple_proposal_runner import create_simple_runner
from proposalAgent.utils.logger import get_logger

logger = get_logger("TestDebateProcess")


def test_debate_with_detailed_state():
    """测试包含详细状态信息的辩论过程"""
...
* **保障机制:** 建立完善的项目管理制度，明确职责分工，建立沟通交流机制，制定风险管理措施，合理配置资源。 [P46]
        ...
        # 配置
        config = {"configurable": {"thread_id": "debate_test_thread"}}
        ...
        if "debate_results" in result:
            ...
            print(f"\n=== 辩论结果分析 ===")
            print(f"辩论结果类型: {type(debate_results)}")
            print(f"辩论涉及学科数量: {len(debate_results)}")
            
            for discipline, debates in debate_results.items():
                print(f"\n学科: {discipline}")
                
                if "feasibility" in debates:
                    feasibility = debates["feasibility"]
                    print("  可行性辩论:")
                    if "good" in feasibility:
                        print(f"    ✓ 正方观点: 已生成")
                    if "bad" in feasibility:
                        print(f"    ✓ 反方观点: 已生成")
                    if "judge" in feasibility:
                        print(f"    ✓ 裁判结论: 已生成")
                
                if "innovation" in debates:
                    innovation = debates["innovation"]
                    print("  创新性辩论:")
                    if "good" in innovation:
                        print(f"    ✓ 正方观点: 已生成")
                    if "bad" in innovation:
                        print(f"    ✓ 反方观点: 已生成")
                    if "judge" in innovation:
                        print(f"    ✓ 裁判结论: 已生成")
        else:
            print("⚠ 未找到辩论结果")
        ...
        print(f"✗ 辩论测试失败: {e}")
        logger.error("辩论测试过程中发生错误: %s", str(e))
        import traceback
        traceback.print_exc()
        return False
...
def main():
    ...
    tests = [
        test_debate_with_detailed_state,
    ]
