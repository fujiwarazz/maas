...
def test_search_with_content():
    """测试带内容解析的搜索功能"""
    print("\n=== 测试带内容解析的搜索功能 ===")
    results = baidu_search_with_content.invoke({"query": "人工智能", "num_results": 2, "depth": 1})
    ...
    if results:
        for i, result in enumerate(results):
            ...
            if 'url_content' in result:
                ...
                print(f"页面标题: {url_content.get('title', 'N/A')[:50]}...")
                print(f"页面内容: {url_content.get('content', 'N/A')[:150]}...")
...
def test_deep_search():
    """测试深层解析功能"""
    print("\n=== 测试深层解析功能 ===")
    results = baidu_search_with_content.invoke({"query": "python教程", "num_results": 1, "depth": 2})
        ...
        if 'url_content' in result:
            ...
            if 'deeper_links' in result:
                ...
                for j, deep_link in enumerate(deeper_links[:2]):  # 只显示前2个
                    ...
                    print(f"  深层链接 {j+1}: {link_info.get('text', 'N/A')[:30]}...")
