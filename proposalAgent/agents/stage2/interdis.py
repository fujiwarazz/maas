...
from proposalAgent.tools.secondary_discipline_rag import third_level_discipline_search,secondary_discipline_search
...
def create_interdis_agent(llm, toolkit):
    ...
    def interdis_agent(state):
        ...
        if research_info and research_body:
            system_prompt = """
            你是一个跨学科领域识别专家。请根据给定的研究基础信息与正文摘要，判断涉及到的领域，可以是多个。

            输出要求：
            - 仅输出以中文领域名称组成的列表，使用逗号分隔，不要包含其他解释性文字。
            
            **注意** 你只可以使用一次工具！ 一次性传入所有你认为的领域，工具一次返回所有的对齐后的二级学科名称，不要分多次调用工具！
           ...
           # prompt = prompt.partial(research_body=research_body)
            ...
            if len(result.tool_calls) == 0:
                ...
                interdis_report = interdis_report[:3]
            else:
