...
def human_review_node(state: AgentState) -> AgentState:
    ...
    print(f"   质量评分: {completeness_result.get('overall_quality', 'N/A')}")
    ...
    print(f"📝 收到人类反馈: {state['human_feedback']}")
    return state
...
def _route_after_feedback(state: AgentState) -> str:
    ...
    print(f"🔄 反馈分析路由决策: {routing_decision}")
...
async def main():
    ...
    try:
        # 运行工作流
        config = {"configurable": {"thread_id": "stage3_complete_test_thread"}}
        result = await graph.ainvoke(state, config=config)
        
        print("\n" + "="*60)
        print("🎉 Stage3完整工作流测试完成!")
        print("="*60)
        
        # 显示详细结果
        print(f"\n📋 最终结果详情:")
        print(f"   完备性检查建议: {result.get('completeness_recommendation', 'N/A')}")
        print(f"   人类反馈: {result.get('human_feedback', 'N/A')}")
        print(f"   反馈分析路由: {result.get('feedback_routing_decision', 'N/A')}")
        print(f"   最终报告长度: {len(result.get('final_report', ''))}")
        print(f"   反思结果: {result.get('reflection_decision', 'N/A')}")
        
        # 显示完备性检查结果
        completeness_result = result.get('completeness_check_result', {})
        ...
        # 显示反馈分析结果
        feedback_result = result.get('feedback_analysis_result', {})
        if feedback_result:
            print(f"\n🧠 反馈分析详情:")
            print(f"   问题类型: {feedback_result.get('problem_type', 'N/A')}")
            print(f"   优先级: {feedback_result.get('priority_level', 'N/A')}")
            print(f"   下一步: {feedback_result.get('next_step', 'N/A')}")
            print(f"   具体指令: {feedback_result.get('specific_instructions', 'N/A')}")
